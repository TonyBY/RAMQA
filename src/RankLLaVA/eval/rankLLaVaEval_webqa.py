    ...
    if _is_peft_model(model) and args.init_checkpoint:
        ...
        if hasattr(model, "active_adapter") and hasattr(model, "load_adapter"):
            if os.path.exists(args.checkpoint_path):
                ...
                logger.info(
                    "WARNING: The intermediate checkpoints of PEFT may not be saved correctly, "
                    f"consider using a custom callback to save {ADAPTER_WEIGHTS_NAME} in corresponding saving folders. "
                    "Check some examples here: https://github.com/huggingface/peft/issues/96"
                )
        else:
            logger.info("WARNING: Could not load adapter model, make sure to have `peft>=0.3.0` installed")
    ...
    for item in tqdm(first_hop_search_results, desc=f"Evaluating Multi-Modal Reranker: {args.model_type}"):    
        ...
        for evi in evidence_list:
            ...
            if evi_title and evi_text:
                ...
                raise Exception(f"There is no text part in evi: {evi}")
            ...
            if evi_type != 'img':
                ...
                try:
                    inputs =  (str(question), int(evi_id), str(evi_text))
...
def get_scores(first_hop_reranking_results):
    ...
    ##### Evaluating Results  ########################################################################################################
    ##################################################################################################################################
    max_evidence = [i + 1 for i in range(20)]
